...
def send_apology_message(db: Session):
    # Get distinct employer numbers
    distinct_employers = db.query(models.worker_employer.c.employer_number).distinct().all()
    template_name = "apology_message"

    # whatsapp_message.send_greetings(917665292549, template_name)
    for employer in distinct_employers:
        whatsapp_message.send_greetings(employer.employer_number, template_name)
...
def send_audio_message(text : str, user_language : str, employerNumber : int):
    ...
    if user_language == "en-IN" or user_language is None or user_language == "en":
        return send_audio(text, employerNumber, user_language)
        #return send_audio_sarvam(text, employerNumber, "en-IN")
    else:
        translated_text = translate_text_sarvam(text, "en-IN", user_language)
        return send_audio(translated_text, employerNumber, user_language)
...
def generate_employment_contract(employerNumber: int, workerNumber : int, upi : str, accountNumber : str, ifsc : str, panNumber: str, name : str, salary : int, db : Session):
    ...
    whatsapp_message.employer_contract_template(
        employerNumber=employerNumber, 
        worker_name=name, 
        employment_contract_name=employment_contract_name, 
