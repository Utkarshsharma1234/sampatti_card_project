...
def generate_salary_records_all_worker(employerNumber: int, db : Session):
    ...
    try:
        ...
        workers_query = (
            db.query(
                models.Domestic_Worker,
                models.worker_employer.c.date_of_onboarding
            )
            .join(models.worker_employer)
            .filter(models.worker_employer.c.employer_number == employerNumber)
        )
        ...
        for worker_index, worker in enumerate(workers):
            ...
            salary_records = (
                db.query(
                    (func.rtrim(models.SalaryDetails.month) + ' ' + func.cast(models.SalaryDetails.year, String)).label('Month'),
                    models.SalaryDetails.salary,
                    models.SalaryDetails.cashAdvance.label('Month_Cash_Advance'),
                    models.SalaryDetails.repayment.label('Month_Repayment'),
                    models.SalaryDetails.bonus.label('Month_bonus'),
                    models.SalaryDetails.attendance.label('Month_attendance'),
                    models.SalaryDetails.totalAmount.label('Salary_Paid')
                )
                .filter(
                    models.SalaryDetails.worker_id == worker['worker_id'],  # Use worker_number
                    models.SalaryDetails.employerNumber == employerNumber
                )
                .order_by(models.SalaryDetails.year.desc(), cast(models.SalaryDetails.month, Integer).desc()) 
                .all()
            )
    ...
    try:
        ...
        worker_query = (
            ...
            .filter(
                ...
                models.Domestic_Worker.name == workerName
            )
            .first()
        )
        ...
        salary_records = (
            ...
            .filter(
                models.SalaryDetails.worker_id == worker.id,
                models.SalaryDetails.employerNumber == employerNumber
            )
            .order_by(models.SalaryDetails.year.desc(), cast(models.SalaryDetails.month, Integer).desc()) 
            .all()
